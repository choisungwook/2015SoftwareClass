...
void printBranches(int branchLen, int nodeSpaceLen, int startLen, int nodesInThisLevel, const deque<BTreeNode*>& nodesQueue, ostream& out) {
	deque<BTreeNode*>::const_iterator iter = nodesQueue.begin();
...
void printNodes(int branchLen, int nodeSpaceLen, int startLen, int nodesInThisLevel, const deque<BTreeNode*>& nodesQueue, ostream& out) {
	deque<BTreeNode*>::const_iterator iter = nodesQueue.begin();
...
void printLeaves(int indentSpace, int level, int nodesInThisLevel, const deque<BTreeNode*>& nodesQueue, ostream& out) {
	deque<BTreeNode*>::const_iterator iter = nodesQueue.begin();
	for (int i = 0; i < nodesInThisLevel; i++, iter++) {
...
void printPretty(BTreeNode *root, int level, int indentSpace, ostream& out) {
	...
	for (int r = 1; r < h; r++) {
		...
		for (int i = 0; i < nodesInThisLevel; i++) {
			BTreeNode *currNode = nodesQueue.front();
